...
for (const s of SERIES) {
    ...
    // Staleness must be judged against how often the series PUBLISHES. A flat
    // 30-month cutoff silently dropped every annual series 2.5 years into its
    // normal publication cycle — the WID wealth shares, the Gini, real median
    // income — all current data that simply arrives once a year with a lag.
    // Allow 3x the series' own typical observation gap, floored at the flat
    // cutoff so fast series keep the strict rule.
    const gaps = pts.slice(-6).map((p, i, a) => i ? monthsBetween(a[i - 1][0], p[0]) : 0).filter(Boolean).sort((a, b) => a - b);
    const typGap = gaps[Math.floor(gaps.length / 2)] || 1;
    const allow = Math.max(STALE_DROP_MONTHS, typGap * 3);
    if (staleMonths > allow) { dropped.push(`${s.label} (stale: ends ${lastYm})`); continue; }
...
const DIR = {
    ...
    WID_GB_T1: 'bad', WID_GB_T10: 'bad', WID_JP_T1: 'bad', WID_JP_T10: 'bad', WID_AU_T1: 'bad', WID_AU_T10: 'bad',
};
...
const EXTREME_NOTES = {
    ...
    VIXCLS: { high: 'VIX spikes above the 90th percentile historically clustered near panic LOWS in stocks, not tops — fear peaks late.', low: 'Multi-year VIX lows = complacency; forward returns from here were historically mediocre.',
        playHigh: 'Historically: contrarian longs scale into equities during VIX spikes (never all at once — spikes can double); vol sellers wait for the backwardation to break.', playLow: 'Historically: cheap-vol regimes are where hedges/protective puts are bought, not sold; momentum longs ride but tighten stops.' },
    BAMLH0A0HYM2: { high: 'Credit spreads at crisis extremes (2008, 2020) historically preceded strong multi-year bond AND stock returns — max pessimism gets priced.', low: 'Spreads this tight leave no cushion; historically preceded below-average returns.',
        playHigh: 'Historically: crisis spreads were the generational long in credit — buying quality junk at 8-10% spreads paid for years.', playLow: 'Historically: up-in-quality rotation — swap junk for investment grade/treasuries; the asymmetric short is credit protection, but carry bleeds while you wait.' },
    UNRATE: { high: 'Unemployment peaks historically came at or after recession END — extreme highs were early-cycle buy zones in hindsight.', low: 'Multi-decade unemployment lows are late-cycle by definition; little labor slack left to fuel growth.',
        playHigh: 'Historically: maximum-pain unemployment prints were historically when early-cycle equity longs began.', playLow: 'Historically: late-cycle posture — trim leverage, favor quality, keep the shopping list ready.' },
    UMCSENT: { low: 'Consumer-sentiment troughs are a famous contrarian marker: forward 12m equity returns from sentiment lows were historically well above average.', high: 'Euphoric sentiment historically preceded modest forward returns.',
        playLow: 'Historically: one of the better-documented contrarian LONG prompts — misery troughs historically beat euphoria for forward returns. Shorting despair has been the losing side.', playHigh: 'Historically: euphoria = trim, rebalance, raise cash slowly.' },
    ICSA: { low: 'Claims at generational lows = tightest labor market; watch for the upturn — a sustained +15-20% rise off the low historically led recessions.', high: 'Claims spikes mark recessions in progress; the PEAK historically came near the stock market bottom.',
        playLow: 'Historically: stay long but define the exit NOW — the tradable signal is the sustained upturn off the low, not the low itself.', playHigh: 'Historically: claims peaking = historically near the equity bottom; contrarian longs watched for the rollover.' },
    CPIAUCSL: { high: 'Inflation extremes forced Fed tightening cycles (1974, 1980, 2022) — the playbook regime where cash/short bonds beat risk until the break.', low: 'Deflation-adjacent readings historically meant easy money ahead.',
        playHigh: 'Historically: inflation regimes favored real assets/energy longs and SHORT long-duration bonds until the Fed wins; the turn (CPI rolling over) flips it to duration longs.', playLow: 'Historically: disinflation = duration longs and growth-equity tailwinds.' },
    HOUST: { low: 'Housing-start collapses historically marked the depth of recessions — and the rebound led the recovery.', high: 'Construction booms at extremes preceded the 2006-type overhangs.',
        playLow: 'Historically: homebuilder longs from start-collapse extremes were among the great early-cycle trades (2011-12).', playHigh: 'Historically: supply overhang building — the eventual short thesis writes itself, but timing needed rate pressure too.' },
    DCOILWTICO: { high: 'Oil price extremes acted as a consumer tax — 1974/1980/2008 spikes preceded recessions and their own demand-destruction crash.', low: 'Oil collapses (1986, 2015, 2020) squeezed producers but acted as a consumer stimulus with a lag.',
        playHigh: 'Historically: ride energy longs with stops, rotate OUT of transports/consumers; the spike eventually kills its own demand — fade only after the trend breaks.', playLow: 'Historically: consumer/transport longs benefit; energy-producer longs waited for capex cuts to bite supply.' },
    FEDFUNDS: { high: 'Peak policy rates historically marked the best long-bond entry points of each cycle.', low: 'Rates at generational lows = cheap-money era; historically inflated valuations and preceded hiking cycles.',
        playHigh: 'Historically: when rates peak, long-term bonds bought at those yields have historically been the standout trade, because their prices rise as rates come back down.', playLow: 'Historically: lock in cheap fixed borrowing while it lasts; bonds have little left to gain from here, and protection against rates rising is cheap.' },
    PCOPPUSDM: { high: 'Copper at record extremes historically reflected demand booms or supply crunches — "Dr. Copper" ran hot before overheating episodes.', low: 'Copper collapses tracked global recessions (2008, 2015, 2020).',
        playHigh: 'Historically: momentum longs respect all-time highs (shorting records = squeeze bait); miners as the levered expression; the exit cue is growth data rolling over.', playLow: 'Historically: copper capitulation lows were early-cycle entry markers for industrial/EM longs.' },
    YH_GOLD: { high: 'Gold at the 90th+ percentile historically meant falling real rates, policy distrust, or crisis hedging (1979-80, 2011, 2020).', low: 'Gold at multi-year lows accompanied strong-dollar, high-real-rate regimes.',
        playHigh: 'Historically: a high price is not by itself a reason to bet against gold — that has bankrupted people for years at a stretch. Those who stayed with the trend used a rule to exit rather than a price target, and waited for inflation-adjusted rates to turn up before betting the other way.', playLow: 'Historically: gold lows during rate-cutting cycles were where long-term holders accumulated.' },
    YH_SILVER: { high: 'Silver extremes ride gold + industrial demand — historically spikier and faster to reverse than gold (1980, 2011).', low: 'Silver washouts tracked industrial recessions.',
        playHigh: 'Historically: same as gold but with tighter risk control — silver retraces harder; parabolic moves historically gave back 30-50% within two years.', playLow: 'Historically: high-beta metal recovery long once gold trend confirms.' },
    CP0000EZ19M086NEST: { high: 'Eurozone inflation extremes force ECB hawkishness.', low: 'EZ disinflation historically brought ECB easing.',
        playHigh: 'Historically: European duration shorts / EUR rate-hike positioning until the print rolls over.', playLow: 'Historically: European duration longs.' },
    YH_CL: { high: 'Front-month crude at extremes — same consumer-tax mechanics as spot WTI, but this is the live traded price.', low: 'Crude futures collapsed — producer pain, consumer stimulus with a lag.',
        playHigh: 'Historically: energy longs ride with stops; transports/consumer shorts are the pair; fade only after trend break — demand destruction is the eventual assassin.', playLow: 'Historically: consumer/transport longs; energy-producer longs wait for supply capitulation (capex cuts, rig counts falling).' },
    YH_HG: { high: 'Copper futures at extremes — "Dr. Copper" pricing either a demand boom or a supply crunch.', low: 'Copper futures washouts tracked global recessions.',
        playHigh: 'Historically: momentum respects records (shorting them = squeeze bait); miners (XME) as the levered vehicle; exit cue = growth data rolling over.', playLow: 'Historically: capitulation lows were early-cycle industrial entry points.' },
    YH_ZW: { high: 'Wheat at real-term extremes = food inflation pressure — historically fed headline CPI and EM instability.', low: 'Cheap grain eased food inflation.',
        playHigh: 'Historically: agricultural exposure follows the trend, while packaged-food companies get squeezed, because grain is their raw material and they cannot always raise prices fast enough.', playLow: 'Historically: cheap grain is a cost tailwind for food processors and consumer staples.' },
    YH_ZC: { high: 'Corn at real-term extremes — same food-inflation channel as wheat, plus ethanol/fuel linkage.', low: 'Cheap corn = feed/food cost relief.',
        playHigh: 'Historically: as wheat — ag trend longs, processor margin caution.', playLow: 'Historically: protein producers (feed costs) and staples benefit.' },
    NFCI: { high: 'Financial conditions at tight extremes — credit crunches (2008, 2020) live here.', low: 'Conditions at historic ease — cheap money everywhere, historically fuel for bubbles.',
        playHigh: 'Historically: tight-conditions extremes were scale-in points for quality risk assets — max tightness clusters near bottoms.', playLow: 'Historically: ride the liquidity but tighten stops; conditions this easy eventually invite the tightening that ends the party.' },
    YH_USDJPY: { high: 'Yen at extreme weakness — the global carry trade (borrow cheap yen, buy everything else) at maximum stretch. The Aug 2024 unwind started exactly here.', low: 'Yen extremely strong — carry trades unwinding, global risk assets typically under pressure.',
        playHigh: 'Historically: enjoy risk assets but know the tripwire — a fast 5%+ yen RALLY from extremes historically forced global deleveraging (carry unwind).', playLow: 'Historically: yen-strength spikes accompany the panic phase — a scale-in signal for risk, not a sell signal.' },
    BREADTH: { high: 'Broad participation — the average stock keeping up with the index. Historically the healthy configuration.', low: 'Narrow market — a few giants carrying a sagging average stock. The fragile configuration.',
        playHigh: 'Historically: broad rallies forgave mistakes; cyclical and small-cap longs historically worked.', playLow: 'Historically: narrow leadership late in cycles favored quality/index positions over stock-picking; breadth troughs that TURN UP marked durable bottoms.' },
    DRCCLACBS: { high: 'Card delinquencies at extremes = household stress no sentiment survey can spin.', low: 'Cleanest consumer credit in decades.',
        playHigh: 'Historically: consumer-lender and subprime shorts were the asymmetric side; discounters outperform.', playLow: 'Historically: consumer credit this clean supported lenders and consumer cyclicals — until claims turn.' },
    WFRBST01134: { high: 'Wealth concentration at record extremes — the economy runs on asset prices: the wealth effect does the spending, and policy risk (taxes, tariffs, transfers) stops being a tail scenario.', low: 'Wealth growth broadly shared — consumption tracks wages more than markets.',
        playHigh: 'Historically: consumption follows the S&P more than paychecks — equity drawdowns transmit to spending with a short lag; the luxury+discount barbell beats the middle; populist policy shifts get priced as base case.', playLow: 'Historically: wage-driven cycle — broad consumer cyclicals work evenly.' },
    WFRBSB50215: { low: 'Bottom-half wealth share at record lows — the paycheck economy has no buffer: small shocks (gas, rates, layoffs) transmit to delinquencies fast.', high: 'Bottom-half balance sheets unusually healthy (the 2020-21 stimulus era was the modern peak).',
        playLow: 'Historically: discount retail over mid-tier; subprime credit is the early-warning short when claims turn; transfer-heavy fiscal responses become the base case in any downturn.', playHigh: 'Historically: broad consumer resilience — the median-consumer basket works.' },
    PRS85006173: { low: 'Labor share of output near historic lows — capital is taking a record slice: corporate margins are fat but politically exposed (minimum-wage pushes, unionization, tax shifts).', high: 'Labor share elevated — wages eating into margins; historically a late-cycle profit-squeeze marker.',
        playLow: 'Historically: fat-margin regimes support equities until labor or policy claws it back — the turn signals are wage prints accelerating and unionization waves.', playHigh: 'Historically: margin-compression era — favor pricing power over labor-heavy business models.' },
    COMPU5MUSA: { high: 'Apartments are being finished at a rate rarely seen. New supply is what actually slows rent growth, and 2024 saw the most completions since 1974, alongside rent inflation falling from 8.3% to under 3%.', low: 'Very little new supply arriving, which historically let rents run.',
    ...
    WEALTHGDP: { high: 'The value of everything households own is at a near-record multiple of what the economy produces in a year. It was 3.5x in 1985. When claims grow faster than output, the gap is paid for by higher prices on the same assets rather than by more production, which is why owning assets has beaten earning wages.', low: 'Asset values low relative to the economy, which historically preceded strong long-run returns.',
        playHigh: 'Historically: high starting valuations relative to the economy have historically meant weaker long-run returns and a bigger role for policy, since the wealth is concentrated and marked to prices that must keep rising to hold. It also means asset prices, not wages, drive spending.', playLow: 'Historically: buying when claims are cheap relative to output was historically the better entry.' },
    M2V: { low: 'Each dollar is changing hands less often than almost ever. Read this carefully: the long decline is mostly an artifact of the Fed creating money faster than the economy grew, not proof that people are hoarding.', high: 'Money circulating fast, which historically accompanied inflationary booms.',
        playLow: 'Historically: slow-circulating money has usually meant inflation stays contained despite large money supplies, because the money is sitting still. Watch for the turn: velocity rising while the money supply is already large is the combination that produced inflation.', playHigh: 'Historically: fast money plus a large money supply is the inflationary configuration.' },
    A006RE1Q156NBEA: { low: 'A small share of the economy is going into building things: factories, equipment, housing. Historically that meant slower future growth, and it is one of the fingerprints of capital preferring existing assets over new ones.', high: 'Heavy investment in productive capacity, which historically supported future growth but sometimes built the overcapacity behind the next bust.',
        playLow: 'Historically: when capital buys existing assets rather than building new ones, asset prices rise faster than the economy does, which is how prices detach from earnings.', playHigh: 'Historically: investment booms supported industrials and materials, until the capacity turned out to be too much.' },
    WFRBSTP1300: { high: 'The wealthiest 1 in 1,000 households hold a near-record share of everything. This is the sharpest end of concentration and it moves before the broader top-1% number does.', low: 'Concentration at the very top unusually low.',
        playHigh: 'Historically: at this end of the distribution, spending is almost entirely discretionary and driven by asset prices, so the wealth effect dominates and the political response becomes a live risk.', playLow: 'Historically: broader-based ownership historically meant more stable consumption.' },
    WEALTHGAP: { high: 'The gap at record width — a two-track economy: aggregate consumer stats are top-decile spending in disguise, and the bottom half lives in a different economy than the index.', low: 'Gap narrowing — wage gains outrunning asset gains (rare: late 1990s, 2020-21).',
        playHigh: 'Historically: K-shape positioning — luxury + discounters over the middle; equities become the LEADING consumer indicator (drawdowns hit spending within a quarter); political-risk premium on every policy calendar.', playLow: 'Historically: broad-based cycles favor small caps and the median consumer.' },
    GFDEGDQ188S: { high: 'Debt/GDP at war-era extremes. The post-WW2 exit from 106% was growth (biggest), financial repression (inflation above capped rates quietly taxing bondholders), and primary surpluses held up by 91%-top-rate-era taxation — a package deal that also drove the Great Compression in the wealth charts above.', low: 'Fiscal headroom — countercyclical capacity intact.',
        playHigh: 'Historically: repression-era assets — gold and real assets carry a structural bid; long NOMINAL bonds are the structurally handicapped asset (T-bills reprice with inflation, 30-year bonds eat it). The historical loser of high-debt eras was the long-bond holder, not the stockholder.', playLow: 'Historically: duration is safe; fiscal expansions from low debt fund early-cycle booms.' },
    FYOIGDA188S: { high: 'Interest is eating the budget — the 1980s-90s bond-vigilante era lived here; crowding-out becomes the political story and deficits become self-feeding (borrowing to pay interest).', low: 'Debt service trivially cheap — the 2010s free-money configuration.',
        playHigh: 'Historically: term-premium regimes — short duration + inflation-protected (TIPS) over long nominals; ends when primary deficits actually shrink, not when politicians promise they will.', playLow: 'Historically: cheap service costs let duration and equities both work.' },
    FORSHARE: { high: 'Foreigners fund an outsized slice of the debt — the "kindness of strangers" regime: the dollar and Treasury market are load-bearing walls of the same building.', low: 'Foreign share at generational lows — the debt is being absorbed domestically (Fed, banks, funds, households): the Japan path.',
        playHigh: 'Historically: sensitive to dollar shocks — foreign selling shows up as simultaneous bond+dollar weakness (the 2022 UK gilt lesson at scale).', playLow: 'Historically: domestic-absorption eras rhyme with financial repression — institutions get nudged into holding government paper, and gold historically caught the quiet bid.' },
};
...
// flags the contradictions between them, and describes what has usually worked
...
const TENSIONS = [
    ...
    ['K-SHAPED ECONOMY', 'CONSUMER SQUEEZE', () =>
        `Sentiment is ${N('UMCSENT')} while the top 1% hold ${N('WFRBST01134')}% of wealth and stocks sit near record highs. In the first quarter of 2026 the top 20% of earners increased their spending 6.5% while the bottom 80% managed 2.6%, which is below inflation — four-fifths of the country is buying less in real terms each year. Total spending can therefore hold up on high earners alone while everyone else cuts back. What has historically ended the split is layoffs, because they take income from the people already stretched. Watch jobless claims, currently ${N('ICSA', 0)}k.`],
];
...
if (has('K-SHAPED ECONOMY')) PLAYS.push(['Watch the stock market to predict consumer spending',
    `The richest 1% hold ${S('WFRBST01134') ? S('WFRBST01134').latest.toFixed(1) + '%' : 'a record share'} of US wealth, and high earners do a disproportionate share of the spending. The cleanest measure of that is the growth gap rather than any level estimate: in the first quarter of 2026, spending by the top 20% of earners rose 6.5% while spending by the bottom 80% rose 2.6%, which is less than inflation — so in real terms four-fifths of the country is buying LESS each year. (Level estimates are disputed: Moody's puts the top 10% near 49% of all spending, critics using government survey data argue 35% or even 20%. Every method agrees the share far exceeds their share of the population and is rising.) That flips the usual logic: the stock market becomes an early warning for consumer spending, because when portfolios fall those households cut back within a few months. It also means headlines about a resilient consumer, built on national averages, mostly describe people who own assets. Worth watching: expensive brands and deep-discount chains hold up better than mid-market retailers, and credit card delinquencies (${S('DRCCLACBS') ? S('DRCCLACBS').latest.toFixed(2) + '%' : 'tracked above'}) give the earliest sign that lower-income households are breaking. History says concentration this extreme eventually produces a political response rather than continuing indefinitely: the last comparable peak was 1929, and what followed was the New Deal, top tax rates above 90%, and mass unionization. The reversal after 1980 came the same way, through policy — the top rate falling from 70% to 28% and union membership collapsing. So tariffs, transfers and tax changes belong in the base case, not the tail. SIGNS THIS IS OVER: workers' share of output rising and the wealth gap narrowing for two quarters or more.`]);
...
if (!PLAYS.length) PLAYS.push(['Nothing worth doing right now',
    'No strong pattern in the data at the moment. The answer for that is boring on purpose: stay broadly invested, keep cash earning interest, and wait until the numbers actually say something.']);
...
${(() => {
    ...
    return `<h2>Extremes Watch <span style="color:var(--muted);text-transform:none;letter-spacing:0;">— indicators in the top/bottom 10% of their own history. The green lines describe how investors have historically positioned at these levels — educational framing, not recommendations.</span></h2>
...
${deskPlays.map(([title, body], i) => `<div style="padding:6px 12px;${i < deskPlays.length - 1 ? 'border-bottom:1px solid var(--line);' : ''}">
    <span style="color:var(--good);font-weight:700;">WHAT USUALLY WORKED ${i + 1}: ${esc(title)}</span>
...
${payload.series.some(s => s.group === 'ineqw') ? `<h2>Wealth &amp; Inequality — International <span style="color:var(--muted);text-transform:none;letter-spacing:0;">— the same dials from other countries' central-bank equivalents of the Fed data above: the ECB's Distributional Wealth Accounts (quarterly since 2011) and Statistics Canada's household economic accounts (quarterly since 2010, annual before 2020). The ECB publishes nothing finer than the top 5% and Canada publishes QUINTILES — top 20%, not top 10% — so compare each country with its own past, not number-for-number across countries. The UK, Japan and Australia publish no official machine-readable equivalent, so their series are annual World Inequality Database estimates via Our World in Data — marked (WID), and the UK line reaches back to 1820.</span></h2>
...
const CHART_SECTIONS = [
    ...
    ['ineqw', 'Wealth & Inequality — International'],
    ['debt', 'Sovereign Debt'],
